...
class Main:
  def __init__(self):
      ...
      if user_try == '1':
        ...
        if user_try in self.already_used_letters:
          ...
          if letter_check is not None:
            correct_letters_amount = game.change_hash(letter_check, user_try)
    ...
    if self.is_correct == True and self.has_chances > 0:
      game.default_print(f'You Win, the word was {game.secret_word.capitalize()}, congratulations!!!')
    else:
      game.default_print(f'You Lost, the word was "{game.secret_word.capitalize()}", good luck next time!')
